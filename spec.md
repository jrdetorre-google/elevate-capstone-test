...
> 2. La **Especificación Funcional y Técnica (`SPEC.md`)** lista para ser ingerida directamente por **Google Antigravity 2.0 / CLI** con el fin de construir una aplicación web interactiva de simulación de examen con **selector de idioma y traducción dinámica mediante Gemini API** antes de mostrar las preguntas en pantalla.
> 3. El **Corpus Canónico en Inglés (`English Corpus`) de 150 Preguntas y Respuestas** que cubre el 100% de las 25 presentaciones oficiales de los Módulos 0, 1, 2 y 3, con un **80% de preguntas de única respuesta (120 preguntas)**, un **20% de preguntas multirespuesta (30 preguntas, 2 o 3 correctas, nunca 4)** y una **distribución matemática equiprobable exacta (25,0% `A`, 25,0% `B`, 25,0% `C`, 25,0% `D`)**.
...
## 2. Especificación para Google Antigravity (`SPEC.md`) — Aplicación de Evaluación con Traducción Dinámica vía Gemini

> **Instrucciones para Antigravity**: Construye una aplicación web moderna, rápida y responsiva (en **React + TypeScript + Vite + Tailwind CSS + Lucide Icons + `@google/genai`**) que cargue el **Corpus Canónico en Inglés de 150 preguntas** (desde `elevate_questions_150.json` o parseando la Sección 3 de este documento) y permita al usuario **elegir el idioma en el que desea examinarse**, invocando a **Gemini** para traducir dinámicamente las preguntas, opciones y explicaciones al idioma seleccionado **antes de mostrarlas en pantalla**.

### 2.1 Selección de Idioma y Flujo de Traducción en Tiempo Real con Gemini (`Pre-Render Translation Pipeline`)
...
### 2.2 Implementación de Referencia del Servicio de Traducción con Gemini (`geminiTranslator.ts`)
...
export async function translateQuestionsBeforeRender(
  ...
  // 2. Translate uncached questions in chunks of 6 before rendering on screen
  const chunkSize = 6;
...
### 2.3 Modos de Examen Soportados
...
3. **Full 150-Question Marathon Mode**:
   - Permite realizar las 150 preguntas con guardado automático de estado en `localStorage`.
4. **Study / Practice by Module Mode (Module 0, 1, 2, or 3)**:
   - Permite filtrar por módulo o lección, traduciendo antes de mostrar en pantalla y ofreciendo un botón **"Check Answer"** que revela al instante las opciones correctas en verde, las incorrectas en rojo y la explicación técnica traducida al idioma del usuario.

### 2.4 Reglas de Interfaz y Puntuación (`Single-Choice` vs. `Multi-Response`)
- **Single-Choice Questions (`type: "single"`, `num_correct: 1`)**:
  ...
  - Mostrar badge (traducido al idioma activo): `Single-Choice (Select 1 option)`.
- **Multi-Response Questions (`type: "multi"`, `num_correct: 2` o `3`)**:
  ...
  - Validar en la UI que el usuario haya marcado exactamente `num_correct` casillas (`Selected: k / X`).
  - **Regla de puntuación estricta (Capstone Standard)**: Una pregunta multirespuesta otorga **1 punto** únicamente si el conjunto de opciones seleccionadas coincide exactamente con el array `correct`.

---

## 3. Canonical English Question Corpus — 150 Questions & Answers (Modules 0, 1, 2 & 3)
